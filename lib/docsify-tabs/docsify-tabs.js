...
(function() {
        ...
        tabBlocks.forEach((function(tabBlock, index) {
            ...
            if (!activeButton) {
                ...
                if (!activeButton && settings.persist) {
                    activeButton = Array.apply(null, tabBlock.children).filter((function(elm) {
    ...
    function setActiveTabFromAnchor() {
        var uriComponent = window.location.hash || window.location.search;
        var anchorID = decodeURIComponent((uriComponent.match(/(?:id=)([^&]+)/) || [])[1]);
